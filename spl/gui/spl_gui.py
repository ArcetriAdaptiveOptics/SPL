...
from spl.SPL_data_processer import SplProcessor
from spl.SPL_data_analyzer import SplAnalyzer
from spl.conf import configuration as config
from spl import devices # Import the new devices module
from spl.calib.calibrate_exposure_time import CalibrateExposureTime
...
class SplGuiApp:
    ...
    def _create_daemon_thread(self, target, *args, **kwargs):
        """Helper function to create a daemon thread with proper cleanup."""
        thread = threading.Thread(target=target, args=args, kwargs=kwargs)
        thread.daemon = True
        # Add a stop flag to the thread
        thread._stop = False
        return thread
    ...
    def run_acquisition(self):
        ...
        try:
            ...
            # Start the acquisition thread using the helper function
            self._create_daemon_thread(acquisition_thread).start()
    ...
    def _create_calibration_widgets(self):
        ...
        frame = ttk.LabelFrame(self.tab_calibration, text="Exposure Time Calibration")
        frame.pack(padx=10, pady=10, fill='x')
        ...
        # Create a frame for the button and progress bar
        control_frame = ttk.Frame(self.tab_calibration)
        control_frame.pack(pady=10, fill='x')

        # Add Run button
        button_frame = ttk.Frame(control_frame)
        button_frame.pack(pady=(0, 10))
        run_button = ttk.Button(button_frame, text="Run Calibration", command=self.run_calibration)
        run_button.pack()

        # Add progress bar
        self.calibration_progress_frame = ttk.Frame(control_frame)
        self.calibration_progress_frame.pack(fill='x', padx=10)
        self.calibration_progress_var = tk.DoubleVar()
        self.calibration_progress_bar = ttk.Progressbar(self.calibration_progress_frame, variable=self.calibration_progress_var, maximum=100)
        self.calibration_progress_bar.pack(fill='x', expand=True)
        self.calibration_progress_label = ttk.Label(self.calibration_progress_frame, text="")
        self.calibration_progress_label.pack()

        # Create frame for calibration plots
        self.calibration_plots_frame = ttk.LabelFrame(self.tab_calibration, text="Calibration Results")
        self.calibration_plots_frame.pack(padx=10, pady=10, fill='both', expand=True)
        
        # Create matplotlib figure and canvas for calibration plots
        self.calibration_figure = Figure(figsize=(10, 8), dpi=100)
        self.calibration_canvas = FigureCanvasTkAgg(self.calibration_figure, master=self.calibration_plots_frame)
        self.calibration_canvas.get_tk_widget().pack(side=tk.TOP, fill='both', expand=True)
        
        # Add toolbar for navigation
        from matplotlib.backends.backend_tkagg import NavigationToolbar2Tk
        toolbar = NavigationToolbar2Tk(self.calibration_canvas, self.calibration_plots_frame)
        toolbar.update()

    def run_calibration(self):
        """Run the exposure time calibration process."""
        tt_value = self.tt_var.get()
        if not tt_value:
            messagebox.showerror("Error", "Tracking Number (tt) is required for calibration.")
            return

        self._log_message(f"Starting exposure time calibration for tt: {tt_value}")
        ...
        try:
            ...
            calibrator = CalibrateExposureTime(tt=tt_value)
            
            # Process files and calculate factors
            self._log_message("Processing files and measuring fluxes...")
            calibrator.process_files()
            
            # Plot fluxes
            self._log_message("Plotting flux measurements...")
            flux_plot_path = calibrator.plot_fluxes()
            if flux_plot_path:
                self._log_message(f"Flux plot saved to: {flux_plot_path}")
                # Display the plot
                ax1 = self.calibration_figure.add_subplot(211)
                img1 = plt.imread(flux_plot_path)
                ax1.imshow(img1)
                ax1.axis('off')
                ax1.set_title("Flux vs. Wavelength")
            
            # Calculate correction factors
            self._log_message("Calculating exposure correction factors...")
            calibrator.calculate_exposure_correction_factors()
            calibrator._calculate_mean_correction_factors_per_wavelength()
            
            # Plot mean correction factors
            self._log_message("Plotting mean correction factors...")
            corr_plot_path = calibrator.plot_mean_correction_factors()
            if corr_plot_path:
                self._log_message(f"Correction factor plot saved to: {corr_plot_path}")
                # Display the plot
                ax2 = self.calibration_figure.add_subplot(212)
                img2 = plt.imread(corr_plot_path)
                ax2.imshow(img2)
                ax2.axis('off')
                ax2.set_title("Mean Correction Factors vs. Wavelength")
            
            # Save results to FITS
            self._log_message("Saving results to FITS file...")
            output_fits_path = calibrator.save_results_to_fits()
            if output_fits_path:
                self._log_message(f"Calibration results saved to: {output_fits_path}")
            
            # Update the canvas
            self.calibration_figure.tight_layout()
            self.calibration_canvas.draw()
            
            self._log_message(f"Calibration for tt: {tt_value} complete.")
            messagebox.showinfo("Calibration Complete", f"Calibration for tt: {tt_value} finished.")

        except Exception as e:
            messagebox.showerror("Error", f"An error occurred during calibration: {e}")
            self._log_message(f"Calibration Error: {e}")
    ...
    def on_closing(self):
        if messagebox.askokcancel("Quit", "Do you want to quit?"):
            ...
            try:
                ...
                if hasattr(self, 'acquisition_running') and self.acquisition_running:
                ...
                for thread in threading.enumerate():
                    if thread != threading.main_thread() and not thread.daemon:
                        ...
                        # Set a flag to stop the thread if it's one of our known threads
                ...
                # Log running threads for debugging
                threads = threading.enumerate()
                logger.info("Active threads at shutdown:")
...
if __name__ == '__main__':
    ...
    try:
        ...
        app.on_closing() # Attempt graceful shutdown
    finally:
